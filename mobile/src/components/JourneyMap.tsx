...
import { JourneySegment, Location, Amenity } from '../types/api';
...
interface JourneyMapProps {
  ...
  washrooms: Amenity[];
  onAmenityPress?: (amenity: Amenity) => void;
}
...
export default function JourneyMap({ 
  ...
  const handleMessage = (event: any) => {
    try {
      ...
      if (message.type === 'amenity_press' && onAmenityPress) {
        onAmenityPress(message.data);
      }
    } catch (e) {
  ...
  const html = useMemo(() => {
    ...
    // Convert washrooms to JSON string
    const washroomsJson = showWashrooms ? JSON.stringify(washrooms) : '[]';
...
<body>
  ...
  <script>
    ...
    // Add Amenities / Washrooms markers
    const washrooms = ${washroomsJson};
    if (washrooms && washrooms.length > 0) {
      washrooms.forEach(function(amn) {
        let emoji = '🚽';
        if (amn.type === 'HOSPITAL') emoji = '🏥';
        else if (amn.type === 'PHARMACY') emoji = '💊';

        let hoursText = amn.is_24_7 ? '<span style="color:#059669;font-weight:bold">Open 24/7</span>' : '<span style="color:#6b7280">Standard Hours</span>';
        let staleText = amn.is_stale ? '<div style="color:#d97706;font-size:10px;font-weight:bold;margin-top:2px">⚠️ May Be Stale</div>' : '';

        const amnIcon = L.divIcon({
          html: '<div style="background:#0284c7;width:24px;height:24px;border-radius:50%;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:12px;cursor:pointer">' + emoji + '</div>',
          iconSize: [24, 24], iconAnchor: [12, 12], className: ''
        });

        const marker = L.marker([amn.lat, amn.lon], { icon: amnIcon }).addTo(map);
        
        marker.bindPopup(
          '<div style="font-family:sans-serif;font-size:12px;color:#1f2937">' +
          '<div style="font-weight:bold;font-size:13px">' + amn.name + '</div>' +
          '<div style="margin-top:2px;font-size:11px;color:#4b5563">' + amn.type + ' • ' + hoursText + '</div>' +
          staleText +
          '</div>'
        );
  ...
  }, [origin, destination, segments, selectedSegmentId, showWashrooms, washrooms]);
