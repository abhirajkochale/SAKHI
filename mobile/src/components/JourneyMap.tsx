...
const getMapHtml = (apiKey: string) => `<!doctype html><html><head><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" /><link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" /><style>html,body,#map{height:100%;width:100%;margin:0}.leaflet-control-attribution{font-size:9px}</style></head><body><div id="map"></div><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><script>
const map=L.map('map',{zoomControl:true,attributionControl:true}).setView([20.5937,78.9629],4);L.tileLayer('https://api.maptiler.com/maps/streets-v2/256/{z}/{x}/{y}.png?key=${apiKey}',{maxZoom:19,attribution:'\\u0026copy; \\u003ca href="https://www.maptiler.com/copyright/" target="_blank"\\u003eMapTiler\\u003c/a\\u003e \\u0026copy; \\u003ca href="https://www.openstreetmap.org/copyright" target="_blank"\\u003eOpenStreetMap contributors\\u003c/a\\u003e'}).addTo(map);let routeLayer=L.layerGroup().addTo(map),amenityLayer=L.layerGroup().addTo(map);const post=(message)=>window.ReactNativeWebView.postMessage(JSON.stringify(message));const stop=(event)=>{if(event&&event.originalEvent)L.DomEvent.stopPropagation(event.originalEvent)};const riskColor=(risk)=>risk>65?'#ef4444':risk>=35?'#f59e0b':'#10b981';
...
export default function JourneyMap(props: JourneyMapProps) {
  ...
  return <View style={styles.container}><WebView ref={mapRef} originWhitelist={['*']} source={{ html: getMapHtml(process.env.EXPO_PUBLIC_MAPTILER_API_KEY || '') }} onMessage={handleMessage} javaScriptEnabled domStorageEnabled startInLoadingState renderLoading={() => <Text style={styles.loading}>Loading Map…</Text>} /><View pointerEvents="none" style={styles.legend}><Text style={styles.legendTitle}>SAKHI RISK MAP</Text><Text style={styles.legendText}>● Green low   ● Amber moderate   ● Red high</Text>{props.showWashrooms && <Text style={styles.legendText}>● Purple washroom</Text>}</View></View>;
}
