...
interface JourneyMapProps { origin: Location | null; destination: Location | null; segments: JourneySegment[]; selectedSegmentId: string | null; onSegmentPress: (segment: JourneySegment) => void; publicToilets: PublicToilet[]; showPublicToilets: boolean; onNavigateRequest: () => void; }
...
export default function JourneyMap(props: JourneyMapProps) {
  ...
  const sendUpdate = useCallback(() => {
    const shouldFitBounds = lastRouteKey.current !== routeKey
      || (props.showPublicToilets && (lastToiletVisibility.current !== props.showPublicToilets || lastToiletCount.current !== props.publicToilets.length));
    mapRef.current?.injectJavaScript(updateScript(props, shouldFitBounds));
    lastRouteKey.current = routeKey;
    lastToiletVisibility.current = props.showPublicToilets;
    lastToiletCount.current = props.publicToilets.length;
  }, [props, routeKey]);
  useEffect(() => { if (ready) sendUpdate(); }, [ready, sendUpdate]);

  const handleMessage = (event: WebViewMessageEvent) => {
    try { const message = JSON.parse(event.nativeEvent.data); if (message.type === 'ready') setReady(true); if (message.type === 'navigate') props.onNavigateRequest(); if (message.type === 'segment') { const segment = props.segments.find((item) => item.segment_id === message.id); if (segment) props.onSegmentPress(segment); } } catch { /* Ignore malformed map messages. */ }
  };
  return <View style={styles.container}><WebView ref={mapRef} originWhitelist={['*']} source={{ html: MAP_HTML }} onMessage={handleMessage} javaScriptEnabled domStorageEnabled startInLoadingState renderLoading={() => <Text style={styles.loading}>Loading OpenStreetMap…</Text>} /><View pointerEvents="none" style={styles.legend}><Text style={styles.legendTitle}>SAKHI RISK MAP</Text><Text style={styles.legendText}>● Green low   ● Amber moderate   ● Red high</Text>{props.showPublicToilets && <Text style={styles.legendText}>● Purple public toilet</Text>}</View></View>;
}
